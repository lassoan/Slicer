  ...
  vtkOrientedImageData* modifierLabelmapInput, ModificationMode modificationMode, const int modificationExtent[6], bool bypassMasking/*=false*/)
{
  ...
  this->referenceGeometryImage();  // ensure we have a valid reference geometry in the segmentation
  bool success = vtkSlicerSegmentationsModuleLogic::ModifySegmentByLabelmap(segmentationNode, segmentID,
    modifierLabelmapInput, static_cast<vtkSlicerSegmentationsModuleLogic::ModificationMode>(modificationMode), modificationExtent,
    bypassMasking ? vtkMRMLSegmentEditorNode::OverwriteNone : this->parameterSetNode()->GetOverwriteMode(),
    bypassMasking ? vtkMRMLSegmentationNode::EditAllowedEverywhere : this->parameterSetNode()->GetMaskMode(),
    bypassMasking ? nullptr : this->parameterSetNode()->GetMaskSegmentID(),
    (bypassMasking || !this->parameterSetNode()->GetSourceVolumeIntensityMask()) ? nullptr : this->parameterSetNode()->GetSourceVolumeIntensityMaskRange(),
    (bypassMasking || !this->parameterSetNode()->GetSourceVolumeIntensityMask()) ? nullptr : this->sourceVolumeImageData());
  if (!success)
  {
    qCritical() << Q_FUNC_INFO << ": vtkSlicerSegmentationsModuleLogic::ModifySegmentByLabelmap failed";
    this->defaultModifierLabelmap();
    return;
  }

  // Make sure the segmentation node is under the same parent as the source volume
  if (this->parameterSetNode()->GetSourceVolumeNode())
  {
    ...
    if (shNode)
    {
      ...
      vtkIdType sourceVolumeShId = shNode->GetItemByDataNode(this->parameterSetNode()->GetSourceVolumeNode());
      ...
      {
        vtkGenericWarningMacro("vtkSlicerSegmentationsModuleLogic::ModifySegmentByLabelmap:"
          " Subject hierarchy items not found for segmentation or source volume");
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
void qSlicerSegmentEditorAbstractEffect::xyzToIjk(double inputXyz[3], int outputIjk[3],
  qMRMLSliceWidget* sliceWidget, vtkOrientedImageData* image, vtkMRMLTransformNode* parentTransformNode/*=nullptr*/)
{
...
//-----------------------------------------------------------------------------
QVector3D qSlicerSegmentEditorAbstractEffect::xyzToIjk(QVector3D inputXyzVector, qMRMLSliceWidget* sliceWidget,
  vtkOrientedImageData* image, vtkMRMLTransformNode* parentTransformNode/*=nullptr*/)
{
  double inputXyz[3] = {inputXyzVector.x(), inputXyzVector.y(), inputXyzVector.z()};
...
//-----------------------------------------------------------------------------
void qSlicerSegmentEditorAbstractEffect::xyToIjk(QPoint xy, int outputIjk[3], qMRMLSliceWidget* sliceWidget,
  vtkOrientedImageData* image, vtkMRMLTransformNode* parentTransformNode/*=nullptr*/)
{
  double xyz[3] = {
    static_cast<double>(xy.x()),
...
//-----------------------------------------------------------------------------
void qSlicerSegmentEditorAbstractEffect::xyToIjk(double xy[2], int outputIjk[3], qMRMLSliceWidget* sliceWidget,
  vtkOrientedImageData* image, vtkMRMLTransformNode* parentTransformNode/*=nullptr*/)
{
...
//-----------------------------------------------------------------------------
QVector3D qSlicerSegmentEditorAbstractEffect::xyToIjk(QPoint xy, qMRMLSliceWidget* sliceWidget,
  vtkOrientedImageData* image, vtkMRMLTransformNode* parentTransformNode/*=nullptr*/)
{
  int outputIjk[3] = {0, 0, 0};
