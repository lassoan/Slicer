...
namespace
{
  const std::string MARKUPS_SCHEMA = "https://raw.githubusercontent.com/slicer/slicer/master/Modules/Loadable/Markups/Resources/markups-schema.json#";
}
...
public:
  ...
  std::string GetMarkupsClassNameFromJsonDocument(rapidjson::Document* jsonRoot);
  bool UpdateMarkupsNodeFromJsonDocument(vtkMRMLMarkupsNode* markupsNode, rapidjson::Document* jsonRoot);
  bool UpdateMarkupsDisplayNodeFromJsonDocument(vtkMRMLMarkupsDisplayNode* displayNode, rapidjson::Document* jsonRoot);
...
bool vtkMRMLMarkupsJsonStorageNode::vtkInternal::ReadVector(rapidjson::Value& item, double v[3], int numberOfComponents/*=3*/)
{
  ...
  if (item.Size() != numberOfComponents)
    {
    return false;
    }
  bool success = true;
...
bool vtkMRMLMarkupsJsonStorageNode::vtkInternal::ReadControlPoints(rapidjson::Value& controlPointsArray, int coordinateSystem, vtkMRMLMarkupsNode* markupsNode)
{
  ...
  for (rapidjson::SizeType controlPointIndex = 0; controlPointIndex < controlPointsArray.Size(); ++controlPointIndex)
    ...
    markupsNode->AddControlPoint(cp);
    }
  return true;
}
//----------------------------------------------------------------------------
bool vtkMRMLMarkupsJsonStorageNode::vtkInternal::UpdateMarkupsNodeFromJsonDocument(vtkMRMLMarkupsNode* markupsNode, rapidjson::Document* jsonRoot)
{
  if (!markupsNode)
    {
  ...
  std::string coordinateSystemStr = (*jsonRoot)["coordinateSystem"].GetString();
  ...
  if ((*jsonRoot).HasMember("locked"))
    {
    markupsNode->SetLocked((*jsonRoot)["locked"].GetBool());
    }
  if ((*jsonRoot).HasMember("labelFormat"))
    {
    markupsNode->SetMarkupLabelFormat((*jsonRoot)["labelFormat"].GetString());
    }

  if ((*jsonRoot).HasMember("controlPoints"))
    {
    if (!this->ReadControlPoints((*jsonRoot)["controlPoints"], coordinateSystem, markupsNode))
      {
      vtkErrorWithObjectMacro(this->External, "vtkMRMLMarkupsJsonStorageNode::vtkInternal::UpdateMarkupsNodeFromJsonDocument failed:"
        << " invalid controlPoints item");
      return  false;
...
//----------------------------------------------------------------------------
bool vtkMRMLMarkupsJsonStorageNode::vtkInternal::UpdateMarkupsDisplayNodeFromJsonDocument(
  vtkMRMLMarkupsDisplayNode* displayNode, rapidjson::Document* jsonRoot)
{
  if (!displayNode)
    {
  ...
  rapidjson::Value& displayItem = (*jsonRoot)["display"];
  ...
  rapidjson::PrettyWriter<rapidjson::FileWriteStream>& writer, vtkMRMLMarkupsNode* markupsNode)
{
  ...
  writer.Key("markupsType"); writer.String(markupsType.c_str());
...
//---------------------------------------------------------------------------
std::string vtkMRMLMarkupsJsonStorageNode::vtkInternal::GetMarkupsClassNameFromJsonDocument(rapidjson::Document* jsonRoot)
{
  std::string markupsType = (*jsonRoot)["markupsType"].GetString();
...
//----------------------------------------------------------------------------
vtkMRMLMarkupsNode* vtkMRMLMarkupsJsonStorageNode::AddNewMarkupsNodeFromFile(const char* filePath, const char* nodeName/*=nullptr*/)
{
  ...
  std::string className = this->Internal->GetMarkupsClassNameFromJsonDocument(jsonRoot);
  std::string markupsType = (*jsonRoot)["markupsType"].GetString();
  ...
  bool success = true;
  success = success && this->Internal->UpdateMarkupsNodeFromJsonDocument(markupsNode, jsonRoot);
  ...
  if (success && markupsNode)
    ...
    if (displayNode && (*jsonRoot).HasMember("display"))
      {
      success == success && this->Internal->UpdateMarkupsDisplayNodeFromJsonDocument(displayNode, jsonRoot);
      }
    }
...
int vtkMRMLMarkupsJsonStorageNode::ReadDataInternal(vtkMRMLNode *refNode)
{
  ...
  bool success = this->Internal->UpdateMarkupsNodeFromJsonDocument(markupsNode, jsonRoot);
